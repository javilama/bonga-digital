...
export const Carousel: React.FC<CarouselProps> = ({
  ...
  const [index, setIndex] = useState<number>(0); // índice actual
  const [isPlaying, setIsPlaying] = useState<boolean>(true); // autoplay
  ...
  const handleMouseEnter = () => { setIsPlaying(false); clearTimer(); };
  const handleMouseLeave = () => { setIsPlaying(true); };

  const goTo = (idx: number) => { setIndex(idx); setIsPlaying(false); };
    ...
    >
      ...
      {showIndicators && slides.length > 1 && (
        <div className="absolute left-1/2 -translate-x-1/2 bottom-3 flex gap-2 mb-5">
          {slides.map((_, i) => (
            <button key={i} onClick={() => goTo(i)} aria-label={`Ir al slide ${i + 1}`} className={`w-3 h-3 rounded-full ${i === index ? 'bg-bonga-yellow' : 'bg-white/60'}`} />
          ))}
        </div>
      )}
    </div>
